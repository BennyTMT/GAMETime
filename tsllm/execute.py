...
def init_options():
    ...
    #         blank_rorder.append(f'{opt}_{series_len}_n_s')
    ...
    # Section-Avai 
    items = ['base'  , '+entity', '+score' , '+timestamp', '+posi' , 'reorder']
    items = ['+timestamp_CoT']
    items = ['none']
    blank_rorder = []
    for sport in [ 'nba' , 'nfl' ]:
        for item in items:
            if 'nba'== sport and item =='+posi' : continue
...
def benchmark_eval(config, task , save_file ,  query_llm):
    opts = TASKOPTS[task]
    global SLOW
    SLOW = False  if config.model.name not in SLOWMODEL else True
    for opt in opts :
