...
def build_avai_prompts(config , datasets):
    ...
    if 'nfl' in config.experiment.data_path:
        sport = 'nfl'
        items = ['base'  , '+entity', '+score' , '+timestamp', '+posi' , 'reorder' , 'none']
        items = ['base'  , '+entity', '+score' , '+timestamp', '+posi' , 'reorder' , 'none']
    elif 'nba' in config.experiment.data_path:
        sport = 'nba'
        items = ['base'  , '+entity', '+score' , '+timestamp', 'reorder']
    else: 
    ...
    for file_name,datas in datasets.items():
        ...
        for opt in items:
            ...
            is_score = f', scores ({team1}-{team2}),' if conditions['score'] else ''
            is_time = 'timestamps, ' if conditions['timestamp'] else ''
            reasoning_prompt = f'''Below is provided {is_time}events{is_score} and win probabilities ({team1}-{team2}).
{event_series}
    ...
    def pick_event_series(datas):
        while True:
            ...
            event_series , ground_truth , steps , score_gap , trends_wp_A =\
                        generate_blank_query(start_index , datas, SERIESLEN )
...
def select_from_real_fake(init_options, sport , conditions , reorder):
    ...
    event_series , ground_truth , steps , team1 , team2= select_with_start_index(init_options , data_path , conditions)
    if not reorder:
        options , ground_truth_label   = generate_options(ground_truth ,  data_path , init_options  , conditions)
    else:
        options , ground_truth_label   = generate_reorder_options(ground_truth ,  data_path , init_options  , conditions)

    return team1 , team2 , event_series , ground_truth_label, options , steps

def select_with_start_index(init_options  , data_path , conditions ):
    ...
    event_series , ground_truth , steps , score_gap , trends_wp_A =\
                generate_blank_query(start_index , datas, SERIESLEN ,  conditions)

    return event_series , ground_truth , ",".join(steps) , datas[0][2].split("_")[0] , datas[0][3].split("_")[0]


def generate_reorder_options(ground_truth ,  data_path , init_options  , conditions):
    options = []
...
def generate_options( ground_truth ,  data_path , init_options  , conditions ):
    options = []
...
def get_sys_message(sport , conditions ):
    ...
    if sport == 'nfl':
        chatgpt_sys_message = f'You are an assistant for NFL football task. We will provide a series of consecutive{is_timestamp} events{is_score} and win probabilities from a NFL football game, though some intermediate events will be missing. You will need to infer the likely events that occurred in the missing intervals.'
    elif sport == 'nba':
        chatgpt_sys_message = f'You are an assistant for NBA basketball task. We will provide a series of consecutive{is_timestamp} events{is_score} and win probabilities from a basketball game, though some intermediate events will be missing. You will need to infer the likely events that occurred in the missing intervals.'
