...
def build_prompts(config , datasets):
    ...
    if 'blank_rorder' in config.experiment.task:
        # build_random_negative(datasets , task = 'blank_rorder' , sport = sport)
        # build_num_events(datasets , task = 'blank_rorder', sport = sport )
        # build_difficulty(datasets , task = 'blank_rorder', sport = sport )
        diff_testing(datasets , task = 'blank_rorder', sport = sport )


def build_difficulty(datasets , task = 'blank_rorder', sport = '' ):

    def generate_options(ground_truth , datasets  , series_len ,   select_pos_trend = None , level=None , seriesA=None):
        options = []
        metrics = []
        while len(options)!=4 : 
...
'''         
            ...
            save_prompt_json(chatgpt_sys_message , reasoning_prompt ,ground_truth  ,task  , f'{sport}_{level}.json' ,  f"{file_name}"  , start_index)
...
'''         
            ...
            print('Label' , ground_truth)
            print('opt ' , opt , 'k' , file_name )
            save_prompt_json(chatgpt_sys_message , reasoning_prompt ,ground_truth  ,task  , f'{sport}_{series_len}.json' ,  f"{file_name}"  , start_index)
